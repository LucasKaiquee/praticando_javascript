...
//resolução
const soma = numeros.reduce((soma, numero) => {
    return soma + numero;
}, 0);
...
const strings = letras.reduce((concatenar, letra) => {
...
const somaIdade = pessoas.reduce((acumulador, pessoa) => {
    if(pessoa.votou) {
...
const precoTotal = listaDeDesejos.reduce((preco, item) => {
